...
def _handle_issuesubmission_files_removal():
    ...
    issue_submissions_to_remove = IssueSubmission.objects.filter(
        status=IssueSubmission.ARCHIVED, date_modified__range=deletion_limit_dt_range)
    ...
    email_limit_dt = now_dt - dt.timedelta(days=25)
    ...
    issue_submissions_to_email = IssueSubmission.objects.filter(
        status=IssueSubmission.ARCHIVED, date_modified__range=email_limit_dt_range)
