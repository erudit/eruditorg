...
class Email:
    ...
    def __init__(
        ...
        self.recipient = (
            recipient if not email_settings.USE_DEBUG_EMAIL else email_settings.DEBUG_EMAIL_ADDRESS
        )
        ...
        with switch_language(self.language):
            ...
            self.subject = (
                self._render_template(subject_template).strip()
                if subject_template
                else force_str(subject)
            )
            self.html_message = self._render_template(html_template)
            self.text_message = (
                self._render_template(text_template)
                if text_template
