...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for subscriber_id in restriction_subscriber_ids:
            ...
            try:
                self._import_restriction_subscriber(subscriber, subscription_qs)
            except ImportException:
                pass
        logger.info("import.finished", **created_objects)

    @transaction.atomic
    def _import_restriction_subscriber(self, restriction_subscriber, subscription_qs):
        logger = structlog.get_logger(__name__)
        logger = logger.bind(
        ...
        for subscription in subscription_qs.all():
            self._import_restriction_subscription(
                subscription, restriction_subscriber, restriction_profile)

    def _import_restriction_subscription(
            self, restriction_subscription, restriction_subscriber, restriction_profile):
        logger = structlog.get_logger(__name__)
        logger = logger.bind(
            subscriber_id=restriction_subscriber.pk,
