...
def generate_casa_token(
        subscription_id=1, token_separator=':', invalid_nonce=False, invalid_message=False,
        invalid_signature=False, payload_separator=':', time_delta=0, ip_subnet='127.0.0.0/24'):
    ...
    return urlsafe_b64encode(nonce) + token_separator.encode() + \
        urlsafe_b64encode(message + signature)
