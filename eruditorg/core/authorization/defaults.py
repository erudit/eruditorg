...
class AuthorizationConfig:
    ...
    can_manage_institutional_subscription = AuthorizationDef(
        "subscriptions:can_manage_institutional_subscription", _("Abonnés institutionnels")
    )
    """
    ...
    can_manage_organisation_members = AuthorizationDef(
        "subscriptions:can_manage_organisation_members", _("Gérer les membres d’un abonnement")
    )
    """
    ...
    can_consult_royalty_reports = AuthorizationDef(
        "subscriptions:can_consult_royalty_reports", _("Consulter les rapports de redevances")
    )
    """
    ...
    def get_choices(cls, include_staff_only=False):
        ...
        for vattr in inspect.getmembers(cls):
            if isinstance(vattr[1], AuthorizationDef) and (
                ...
                authorizations.append((vattr[1].codename, vattr[1].label))
