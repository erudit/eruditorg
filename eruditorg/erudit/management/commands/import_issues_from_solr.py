...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.cairn_url = "https://www.cairn.info/numero.php?ID_REVUE={}&ID_NUMPUBLIE=".format(
            self.journal_cairn_code
        )
        self.query = "RevueID:{} AND Annee:[{} TO {}]".format(
            self.journal_localidentifier, self.year_min, self.year_max
        )
        self.solr_args = {
            "q": self.query,
        ...
        logger.info(
            "import.started", issues_count=len(self.solr_results), msg="importing issues found"
        )
        try:
