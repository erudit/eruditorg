...
class IssueSubmissionUploadForm(IssueSubmissionForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Update some fields
        initial_files = self.instance.last_files_version.submissions.all().values_list(
            "id", flat=True
        )
...
class IssueSubmissionTransitionCommentForm(forms.Form):
    comment = forms.CharField(label=_("Commentaires"), required=False, widget=forms.Textarea)
