...
class IssueSubmissionListView(JournalScopePermissionRequiredMixin, MenuItemMixin, ListView):
    menu_journal = "editor"
    ...
    def has_permission(self):
        ...
        return self.request.user.has_perm(
            "editor.manage_issuesubmission", obj
        ) or self.request.user.has_perm("editor.review_issuesubmission")


class IssueSubmissionDetailView(JournalScopePermissionRequiredMixin, MenuItemMixin, DetailView):
...
class IssueSubmissionCreate(
    IssueSubmissionContextMixin,
    JournalScopePermissionRequiredMixin,
    MenuItemMixin,
    CreateView,
):
    menu_journal = "editor"
...
class IssueSubmissionUpdate(
    ...
    def get_form(self, form_class=None):
        ...
        form.fields["submissions"].widget.set_model_reference(
            "editor.IssueSubmissionFilesVersion", obj.last_files_version.pk
        )
...
class IssueSubmissionApproveView(IssueSubmissionTransitionView):
    ...
    def get_success_message(self):
        ...
        return _(
            ...
            "le {archive_date}. Les fichiers de production seront supprimés."
        ).format(archive_date=date_format(archive_date, "SHORT_DATE_FORMAT"))
...
class IssueSubmissionDeleteView(JournalScopePermissionRequiredMixin, MenuItemMixin, DeleteView):
    force_scope_switch_to_pattern_name = "userspace:journal:editor:issues"
    menu_journal = "editor"
    model = IssueSubmission
    raise_exception = False
