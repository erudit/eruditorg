...
class IssueDetailView(FallbackObjectViewMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        sections1_tree = self._gen_sections_tree_root(section1_grouped)
    ...
    def _gen_sections_tree_root(self, section1_grouped):
        """ Generate the sections tree root """
        return OrderedDict(
            (
                (k[0] or 'nil-{}-{}'.format(
                    str(k[1][0].ordseq), v
                ), k[1])
                for v, k in enumerate(section1_grouped)
            )
        )
