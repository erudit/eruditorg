...
class JournalDetailCheckRedirectView(
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        if "code" in kwargs:
            journal = Journal.legacy_objects.get_by_id_or_404(kwargs["code"])
...
class IssueDetailRedirectView(
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        if "id" in self.request.GET:
            ...
            try:
                issue = Issue.from_fedora_ids(journal.code, localidentifier)
                ...
                if ticket:
                    return "{url}?ticket={ticket}".format(url=redirect_url, ticket=ticket)
                else:
                    return redirect_url
...
def get_issue_from_year_volume_number(
    journal_code: str, year: str, volume: str, number: Optional[str]
) -> Optional[Issue]:
    """
    ...
    try:
        return Issue.objects.get(
            journal__code=journal_code, year=year, volume=volume, number=number
        )
