...
def GET_as_dict(GET, multi_params=None):
    ...
    return {k: GET.getlist(k) if k in multi_params else GET.get(k) for k in GET}
...
def get_search_elements(queryparams, form=None):
    ...
    def elements(t, f, o):
        ...
        return {
            ...
            "operator": o_,
            "str": (
                "({field} : {term})".format(field=f, term=t)
                if o is None
    ...
    for k, v in params.items():
        if (
            not k.startswith("advanced_search_")
            and not k.startswith("basic_search_")
            and not k.startswith("pub_year")
            and k not in code_to_label_fields
            and k in form.fields
            and v
        ):
