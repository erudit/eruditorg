...
class SearchForm(forms.Form):
    ...
    pub_year_start = forms.ChoiceField(
        label=_("De…"),
        widget=forms.Select,
        choices=get_years_range(add_empty_choice=True),
        required=False,
    )
    pub_year_end = forms.ChoiceField(
        label=_("À…"),
        widget=forms.Select,
        choices=get_years_range(reverse=True, add_empty_choice=True),
        required=False,
    )

    available_since = forms.ChoiceField(
        label=_("Dans Érudit depuis"),
        widget=forms.Select,
    ...
    article_types = forms.MultipleChoiceField(
        ...
        widget=forms.CheckboxSelectMultiple,
        choices=ARTICLE_TYPES_CHOICES,
        required=False,
        help_text=_("Ces filtres s’appliquent aux articles savants uniquement."),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        for fkey in [
            "basic_search_term",
            "advanced_search_term1",
            "advanced_search_term2",
            "advanced_search_term3",
            "advanced_search_term4",
            "advanced_search_term5",
        ]:
...
class ResultsFilterForm(forms.Form):
    ...
    filter_funds = forms.MultipleChoiceField(
        label=_("Fonds"),
        help_text=_(
            """Les revues diffusées sur Érudit sont consultables
    ...
    filter_publication_types = forms.MultipleChoiceField(
        ...
        help_text=_(
            """Les revues savantes publient des articles scientifiques révisés
    ...
    article_type_correspondence = {"Compte rendu": ["Compterendu"]}
    ...
    def __init__(self, *args, **kwargs):
        ...
        if aggregations:
            ...
            legacy.group_results_by_field_correspondence(
                aggregations, "article_type", self.article_type_correspondence
            )

            legacy.group_results_by_field_correspondence(
                aggregations, "language", self.language_code_correspondence
            )
            ...
            self.fields["filter_collections"].choices = self._get_aggregation_choices(
                aggregations["collection"], sort_key=lambda x: (x[1], x[0]), sort_reverse=True
            )
            self.fields["filter_authors"].choices = self._get_aggregation_choices(
                aggregations["author"], sort_key=lambda x: (x[1], x[0]), sort_reverse=True
            )
