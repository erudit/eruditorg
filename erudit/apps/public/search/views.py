...
class EruditDocumentListAPIView(ListAPIView):
    ...
    def filter_queryset(self, queryset):
        # Call the ListAPIView.filter_queryset method in order to apply potential filter backends to
        # the initial queryset.
        queryset = super(EruditDocumentListAPIView, self).filter_queryset(queryset)

        # Applies the search engine filter backend in order to get a filtered queryset and an
        # ordered list of EruditDocument localidentifiers.
        self.localidentifiers, queryset = self.search_engine_filter_backend().filter_queryset(
    ...
    def paginate_queryset(self, queryset):
        return self.paginator.paginate_queryset(
            queryset, self.localidentifiers, self.request, view=self)
