...
class TestRestrictionsView:
    ...
    def test_embargo_duration(self):
        ...
        embargo_duration_scientific = root.find(
            f"journal[@code='{journal_scientific.code}']/embargo_duration"
        ).text
        embargo_duration_cultural = root.find(
            f"journal[@code='{journal_cultural.code}']/embargo_duration"
        ).text
...
class TestRestrictionsByJournalView:
    def test_that_journal_and_all_issues_works(self):
        ...
        url = reverse(
            "webservices:restrictionsByJournal", kwargs={"journal_code": journal.localidentifier}
        )
        response = Client().get(url)
        root = etree.fromstring(response.content.decode())
        ...
        embargoed_count = (
            journal.published_issues.count() - journal.published_open_access_issues.count()
        )
    ...
    def test_that_journal_does_not_exist(self):
        url = reverse("webservices:restrictionsByJournal", kwargs={"journal_code": "undefined"})
        response = Client().get(url)
        root = etree.fromstring(response.content.decode())
    ...
    def test_that_journal_without_issues(self):
        ...
        url = reverse(
            "webservices:restrictionsByJournal",
