...
from erudit.test.factories import ThesisFactory, ThesisRepositoryFactory
...
class TestThesisHomeView:
    def test_inserts_collection_information_into_the_context(self, solr_client):
        ...
        ids = [
            t.localidentifier for t in response.context["repository_summaries"][0]["recent_theses"]
        ]
        assert ids == [
            thesis_4.id,
            thesis_3.id,
        ...
        ids = [
            t.localidentifier for t in response.context["repository_summaries"][1]["recent_theses"]
        ]
        assert ids == [
            thesis_8.id,
            thesis_7.id,
...
class TestThesisPublicationYearListView:
    ...
    def test_invalid_page_in_querystring(self, solr_client):
        ...
        solr_client.add_document(thesis)
        url = reverse("public:thesis:collection_list_per_year", args=(thesis.repository.code, 2012))
        url += "?page=10,11"
        response = Client().get(url)
        assert response.status_code == 200
...
class TestCitationExports:
    ...
    def test_no_html_escape_in_collection(self, solr_client):
        ...
        solr_client.add_document(thesis)
        url = reverse("public:thesis:thesis_citation_bib", args=[thesis.id])
        response = Client().get(url)
        content = response.content.decode("utf-8")
        # values hasn't been mangled in the rendering process.
