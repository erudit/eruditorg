...
class TestEruditDocumentSolrFilter(TestCase):
    ...
    def test_can_properly_handle_advanced_queries_using_the_AND_operator(self, mock_get_results):
        ...
        # Run & check
        filt.filter(request)
        self.assertEqual(filt.sqs._q, "((Metadonnees:(test)) AND (TexteComplet:(intro)))")
    ...
    @unittest.mock.patch.object(Query, "get_results")
    def test_can_properly_handle_advanced_queries_using_the_negation_operator(
        ...
        # Run & check
        filt.filter(request)
        self.assertEqual(filt.sqs._q, "((Metadonnees:(test)) AND ((NOT TexteComplet:(intro))))")
    ...
    def test_can_filter_on_publication_years(self, mock_get_results):
        ...
        filt.filter(request)
        self.assertEqual(filt.sqs._q, "(Metadonnees:(test))")
        self.assertEqual(filt.sqs._fq, '(((Annee:"2015")) OR (Annee:"2016"))')
    ...
    def test_can_filter_on_a_publication_year_range(self, mock_get_results):
        ...
        self.assertEqual(filt.sqs._fq, "Annee:[2012 TO 2016]")
    ...
    def test_can_filter_on_document_types(self, mock_get_results):
        ...
        filt.filter(request)
        self.assertEqual(filt.sqs._q, "(Metadonnees:(test))")
        self.assertEqual(filt.sqs._fq, '((TypeArticle_fac:"Article"))')
    ...
    def test_can_filter_on_collections(self, mock_get_results):
        ...
        self.assertEqual(filt_1.sqs._fq, '((RevueAbr:"Arborescences"))')

        self.assertEqual(filt_2.sqs._q, "(Metadonnees:(test))")

        self.assertEqual(filt_2.sqs._fq, '((TitreCollection_fac:"Arborescences"))')
    ...
    def test_can_filter_on_authors(self, mock_get_results):
        ...
        self.assertEqual(filt.sqs._fq, '((Auteur_tri:"firstname, lastname"))')
    ...
    def test_can_filter_on_funds(self, mock_get_results):
        ...
        self.assertEqual(filt_1.sqs._fq, '((Fonds_fac:"Érudit"))')
        self.assertEqual(filt_2.sqs._fq, '((Fonds_fac:"Érudit"))')
    ...
    def test_can_filter_on_publication_types(self, mock_get_results):
        ...
        self.assertEqual(filt_1.sqs._q, "(Metadonnees:(test))")
        self.assertEqual(filt_2.sqs._q, "(Metadonnees:(test))")
        self.assertEqual(filt_1.sqs._fq, '((Corpus_fac:"Culturel"))')
        self.assertEqual(filt_2.sqs._fq, '((Corpus_fac:"Culturel"))')
    ...
    def test_can_filter_on_extra_terms(self, mock_get_results):
        ...
        filt.filter(request)
        self.assertEqual(filt.sqs._q, "(Metadonnees:(test))")
        self.assertEqual(filt.sqs._fq, 'TexteComplet:("foobar")')
    ...
    def test_can_filter_on_disciplines(self, mock_get_results):
        ...
        filt.filter(request)
        self.assertEqual(filt.sqs._q, "(Metadonnees:(test))")
        self.assertEqual(filt.sqs._fq, '((Discipline_fac:"Littérature"))')
    ...
    def test_can_properly_determine_the_order_of_the_results(self):
        ...
        self.assertEqual(EruditDocumentSolrFilter().get_solr_sorting(request), "Annee_tri asc")

        request = RequestFactory().get("/", data={"sort_by": "pubdate_desc"})
        self.assertEqual(EruditDocumentSolrFilter().get_solr_sorting(request), "Annee_tri desc")
    ...
    def test_can_add_correspondences_to_aggregations(self):
        ...
        correspondences = {"Compte rendu": ["Compterendu"]}

        request_get = add_correspondences_to_search_query(
            request, "filter_article_type", correspondences
        )

        assert "Compterendu" in request_get.get("filter_article_type")


@pytest.mark.parametrize("operator, correspondence", [("AND", "ET"), ("OR", "OU"), ("NOT", "SAUF")])
