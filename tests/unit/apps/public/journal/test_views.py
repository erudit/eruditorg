...
class TestJournalDetailView:
    @pytest.mark.parametrize(
        "localidentifier, language, expected_notes",
        [
            ("journal1", "fr", ["foobar"]),
            ("journal1", "en", ["foobaz"]),
    ...
    def test_contributors(self):
        ...
        assert (
            "Rédaction&nbsp;:\n          \n            Marie-Claude Loiselle (Rédactrice en chef)"
            in html
        )
    ...
    def test_available_since_when_issues_are_not_produced_in_the_same_order_as_their_published_date(
        self,
    ):
...
class TestIssueDetailSummary:
            ...
            "groups": [
                {
                    ...
                    "groups": [
                        {"type": "objects", "level": 1, "objects": [articles[0]]},
                        {
                            "type": "subsection",
                            "level": 2,
                            "titles": {"paral": [], "main": "section 1.1"},  # noqa
    ...
    def test_issue_reader_url_in_context_for_cultural_journal_issues(
        self, journal_type, is_published
    ):
        issue = IssueFactory(
    ...
    @pytest.mark.parametrize(
        "is_external,expected_url",
        ((False, "/fr/revues/journal/2021-issue/article/"), (True, "solr_url")),
    )
...
class TestIssueReaderPageView:
    @pytest.mark.parametrize(
        "page, open_access, is_published, ticket, expected_status_code, expected_redirection",
        [
    ...
    def test_issue_reader_page_view(
        self, page, open_access, is_published, ticket, expected_status_code, expected_redirection
    ):
        issue = IssueFactory(
            is_published=is_published,
        ...
        url = reverse(
            "public:journal:issue_reader_page",
            kwargs={
...
class TestIssueRawCoverpageView:
    @pytest.mark.parametrize("is_published", (True, False))
    @unittest.mock.patch("erudit.fedora.views.generic.get_cached_datastream_content")
    def test_cache_unpublished_issue_coverpages(
...
class TestIssueRawCoverpageHDView:
    @pytest.mark.parametrize("is_published", (True, False))
    @unittest.mock.patch("erudit.fedora.views.generic.get_cached_datastream_content")
    def test_cache_unpublished_issue_coverpages_hd(
        self, mock_get_cached_datastream_content, is_published
    ):
        issue = IssueFactory(localidentifier="issue", is_published=is_published)
...
class TestRenderArticleTemplateTag:
    ...
    def test_space_between_two_tags(self):
        ...
        assert (
            '<span class="petitecap">Note 1,</span> <em>avec espace entre deux marquages</em>'
            in ret
        )
...
class TestGoogleScholarSubscribersView:
    @pytest.mark.parametrize(
        "google_scholar_opt_out, expected_subscribers",
        [
            (
                False,
                {
...
class TestGoogleScholarSubscriberJournalsView:
    @pytest.mark.parametrize(
        "google_scholar_opt_out, subscription_id, expected_journal_ids",
        [
            (False, "1", ["journal_1"]),
    ...
    def test_google_scholar_subscriber_journals(
        ...
        assert [
            journal.localidentifier for journal in context.get("journals")
        ] == expected_journal_ids


class TestJournalStatisticsView:
    @pytest.mark.parametrize(
        "is_staff, is_superuser, has_permission",
        [
            (True, True, True),
            (True, False, True),
            (False, True, True),
...
class TestArticleSummaryView:
    @pytest.mark.parametrize(
        "publication_allowed, processing, expected_access_type",
        (
...
class TestArticleBiblioView:
    @pytest.mark.parametrize(
        "publication_allowed, expected_access_type",
        (
...
class TestArticleRawPdfView:
    @pytest.mark.parametrize(
        "publication_allowed, data, expected_access_type",
        (
            (False, {}, ArticleAccessType.content_not_available),
...
class TestArticleMediaView:
    @pytest.mark.parametrize("is_published", (True, False))
    @unittest.mock.patch("erudit.fedora.views.generic.get_cached_datastream_content")
    def test_cache_unpublished_issue_article_medias(
        self, mock_get_cached_datastream_content, is_published
    ):
        article = ArticleFactory(
            localidentifier="article",
