...
class TestLatestIssuesFeed:
    def _get_pubdate(self, date_published):
        return rfc2822_date(
            make_aware(datetime.datetime.combine(date_published, datetime.datetime.min.time()))
        )
    ...
    def test_rss_feed(self):
        ...
        assert (
            "<description>{start_date} - {today}</description>".format(
...
class TestLatestJournalArticlesFeed:
    def test_rss_feed(self):
        ...
        rss = (
            ...
            .get(
                reverse(
                    "public:journal:journal_articles_rss",
                    kwargs={
                        "code": journal.code,
                    },
                )
